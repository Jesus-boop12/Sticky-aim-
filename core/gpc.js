...
export function buildUniversalScript(classProfiles, options = {}) {
  ...
  body.push('            else        { mods_on = TRUE;  blips = 1; }');
  body.push('        }');
  body.push('        if(event_press(DPAD_UP)) blips = wclass + 1;');
  body.push('    }');
  body.push('');
  body.push('    if(holding_second) wclass = second_class; else wclass = primary_class;');
  body.push('');
  body.push('    if(blips > 0 && !combo_running(FEEDBACK)) combo_run(FEEDBACK);');
  body.push('');
  ...
  body.push('            arh = PH_H[idx + 3];');
  body.push('        }');
  body.push('        if(abs(get_val(STICK_RY)) < AR_RELEASE[wclass]) set_val(STICK_RY, lim(get_val(STICK_RY) + arv));');
  body.push('        if(arh != 0 && abs(get_val(STICK_RX)) < AR_RELEASE[wclass]) set_val(STICK_RX, lim(get_val(STICK_RX) + arh));');
  body.push('    } else {');
  body.push('        fire_ms = 0;');
  body.push('    }');
  body.push('');
  if (anyRapid) {
    body.push('    /* ---- rapid fire ---- */');
    body.push('    if(mods_on && RF_ON[wclass] && firing) {');
    body.push('        rf_hold = RF_HOLD[wclass];');
    body.push('        rf_rest = RF_REST[wclass];');
    body.push('        combo_run(RAPID_FIRE);');
    body.push('    } else if(combo_running(RAPID_FIRE)) {');
    body.push('        combo_stop(RAPID_FIRE);');
    body.push('    }');
  ...
  if (anySticky) {
    ...
    body.push(`    if(${stickyCondition(first.sticky.when).replace(/STICKY_ON\[slot\]/, 'STICKY_ON[wclass]')}) {`);
    body.push('        sticky_r = STICKY_R[wclass];');
    body.push('        sticky_ms = STICKY_MS[wclass];');
    body.push('        combo_run(STICKY_AIM);');
    body.push('    } else if(combo_running(STICKY_AIM)) {');
    body.push('        combo_stop(STICKY_AIM);');
    body.push('    }');
